...
public class TableRandom implements IRandom {
    private int seed = 0;
    private int a = 0;
    ...
    private void initArray(){
        ...
        //взято из книги И. Г. Венецкого, В. И. Венецкой «Основные математико-статистические понятия и формулы в экономическом анализе»

        String array = "5489 5583 3156 0835 1988 3912 0938 7460 0869 4420" +
        ...
        for (String number1 : array.split(" ")){
            for(String number2: number1.split(""))
                randomArray.add(number2);
        }
    }
    ...
    public int getNext() {
        seed %= (this.randomArray.size()-bitNumber);
        StringBuilder sValue = new StringBuilder("");

        for(int i = 0; i<bitNumber; ++i)
            sValue.append(randomArray.get(seed+i));

        seed += bitNumber;
        int res = Integer.valueOf(sValue.toString());
        if(res < a || res > b)
            res = getNext();
